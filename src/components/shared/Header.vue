<template>
  <header
    :class="`fixed z-50 h-[82px] w-full theme-surface shadow-md lg:sticky lg:left-0 lg:top-0 lg:transition-transform lg:duration-300 ${show}`"
  >
    <Wrapper class="flex h-[82px] items-center justify-between py-4 md:gap-3 xl:gap-8">
      ...
      <div class="px-1 lg:w-[383px]">
        <ul
          :class="`fixed top-[81px] z-50 flex h-screen w-[250px] flex-col items-start justify-start gap-0 theme-surface p-2 font-srProDisplay text-base font-medium theme-text *:w-full lg:static lg:h-auto lg:w-auto lg:flex-row lg:items-center lg:justify-center lg:gap-3 lg:bg-transparent lg:p-0 lg:text-muted *:lg:text-center ${showMobileMenu} duration-500 ease-linear lg:duration-0`"
        >
          <li
          ...
          <li
            class="flex items-center justify-between p-3 hover:bg-blue-200 lg:gap-[2px] lg:p-0 hover:lg:bg-transparent hover:lg:text-black"
          >
            <router-link to="/categories" class="flex items-center w-full">
