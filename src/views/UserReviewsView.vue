...
onMounted(async () => {
  ...
  try {
    ...
    const reviewsResponse = await axios.get(`http://localhost:8080/api/users/${userId}/productReviews`)
    reviews.value = reviewsResponse.data._embedded ? reviewsResponse.data._embedded.productReviews : reviewsResponse.data
  } catch (err) {
    ...
    <div class="max-w-4xl mx-auto">
      ...
      <div class="text-center mb-12">
        <div v-if="user" class="flex items-center justify-center space-x-4 mb-4">
          ...
          <div>
            <h2 class="text-3xl font-bold text-gray-900 dark:text-white">{{ user.name || 'Usuario' }}</h2>
            <p class="text-gray-600 dark:text-gray-300">{{ user.email || '' }}</p>
          </div>
        </div>
        <h3 class="text-2xl font-semibold text-gray-900 dark:text-white mb-2">Reseñas de Productos</h3>
      ...
      <div v-else class="space-y-6">
        ...
        >
          <div class="p-6">
            ...
            <!-- Stars Rating -->
            <div class="mb-4">
              <span class="text-2xl">{{ getRatingStars(review.rating) }}</span>
            </div>
