...
<template>
  <div :class="pageBackgroundClasses" class="min-h-screen">
      ...
      <div :class="['p-3 rounded-lg shadow-lg sm:p-4', 'theme-debug-bg']">
        ...
        <transition name="fade-debug">
          <div v-show="showDebug" :class="['space-y-2 overflow-y-auto text-xs max-h-40', 'theme-debug-text']">
            ...
            <div><b>Brands (checked):</b> {{productStore.brands.filter(b => b.checked).map(b => b.name).join(', ')}}
            </div>
            ...
            <button @click="console.log('🧪 [TEST] Debug button clicked!'); goToProductDetails(76)"
    ...
    <div class="px-4 py-4 mx-auto max-w-7xl sm:px-6 lg:px-8 lg:py-8">
      ...
      <div class="mb-6 lg:hidden">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h1 :class="textClasses" class="text-lg font-semibold font-srProDisplay sm:text-xl">{{ categoryDisplayName
              }}</h1>
            <p :class="textSecondaryClasses" class="text-sm font-srProDisplay">{{ filteredProducts.length }} products
              found</p>
          </div>
        </div>
        ...
        <div class="flex gap-3">
          <button @click="openMobileFilters" :class="[cardClasses, hoverClasses]"
          ...
          <button @click="showMobileSorting = !showMobileSorting" :class="[cardClasses, hoverClasses]"
            class="flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-lg text-sm font-medium min-h-[48px]">
            <svg :class="textSecondaryClasses" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div v-if="showMobileSorting" :class="cardClasses" class="mt-3 overflow-hidden rounded-lg shadow-lg">
          <button v-for="option in [
            ...
            {{ option.label }}
            <svg v-if="sortBy === option.value" :class="textClasses" class="w-5 h-5 ml-auto" fill="currentColor"
              viewBox="0 0 20 20">
        ...
        <div class="flex-1">
          ...
          <div class="items-center justify-between hidden mb-6 lg:flex">
            <div>
              ...
              <p :class="textSecondaryClasses" class="font-srProDisplay">{{ filteredProducts.length }} products found
              </p>
            </div>
            <div class="flex items-center space-x-4">
              <button v-if="hasActiveFilters()" @click="clearAllFilters" :class="catalogClearFilterClasses"
                class="text-sm underline transition-colors" type="button">
                Clear All Filters
              </button>
          ...
          <div v-else-if="productStore.error"
            class="flex items-center justify-center px-4 py-3 mb-8 space-x-2 text-center text-red-700 bg-red-100 border border-red-400 rounded-lg dark:bg-red-900/20 dark:border-red-500 dark:text-red-300">
          ...
          <div v-else-if="filteredProducts.length === 0" class="py-12 text-center">
            ...
            <button v-if="hasActiveFilters()" @click="clearAllFilters" :class="buttonPrimaryClasses"
            ...
            >
              <div class="absolute z-10 top-3 right-3">
                <button @click.stop="toggleFavorite(product.id)" :disabled="wishlistLoading"
                  ...
                  <svg v-if="!wishlistStore.isProductInWishlist(product.id)" fill="none" stroke="currentColor"
                ...
                <div class="flex items-center justify-center mb-3">
                  <div class="h-[140px] w-full">
                    <img
                      ...
                      :alt="product.name" class="object-cover w-full h-full rounded-md" loading="lazy" />
                  </div>
                </div>
                <div class="flex flex-col flex-1 gap-3">
                  <div class="flex flex-col gap-2">
                    ...
                    <div class="flex items-center justify-center gap-2">
                      <span :class="textClasses" class="text-lg font-semibold font-figtree">{{
          ...
          <div v-if="totalPages > 1" class="flex items-center justify-center space-x-1 lg:space-x-2">
            ...
            <template v-else>
              <button v-if="currentPage > 2" @click="goToPage(1)" :class="catalogPaginationButtonClasses"
                class="px-3 py-2 rounded-md text-sm font-medium min-w-[44px] min-h-[44px] flex items-center justify-center">
                1
              </button>
    ...
    <div v-if="showMobileFilters" class="fixed inset-0 z-50 bg-black bg-opacity-50 lg:hidden"
      @click="closeMobileFilters">
      <div :class="catalogMobileFilterClasses" class="flex flex-col w-full h-full max-w-sm ml-auto overflow-hidden"
        @click.stop>
        <!-- Header -->
          ...
          <div :class="catalogFilterSectionClasses" class="mb-6">
            ...
            <div v-show="!collapsedFilters.brand">
              <div :class="catalogSearchInputClasses" class="flex items-center justify-start gap-2 p-3 mb-4 rounded-lg">
                ...
                <input v-model="brandSearchQuery"
                  :class="[textClasses, 'w-full p-0.5 font-srProDisplay text-sm font-medium outline-none bg-transparent']"
                  type="search" placeholder="Search brands" />
              </div>
              <div class="space-y-4 overflow-y-auto max-h-64">
                <div v-for="brand in filteredBrands" :key="brand.name" class="flex items-center">
                  <input :id="'mobile-brand-' + brand.name" type="checkbox" v-model="brand.checked"
                    :class="catalogCheckboxClasses" class="w-5 h-5 rounded" />
                  <label :for="'mobile-brand-' + brand.name"
                    :class="[catalogFilterLabelClasses, 'ml-3 flex-1 text-sm font-srProDisplay py-2']">
                    {{ brand.name }}
                  </label>
                </div>
              </div>
            ...
            <div class="space-y-2">
              ...
              <div v-if="productStore.brands.some(brand => brand.checked)" class="text-sm">
                ...
                <div class="flex flex-wrap gap-1 mt-1">
                  <span v-for="brand in productStore.brands.filter(brand => brand.checked)" :key="brand.name"
        ...
        <div :class="[cardClasses, 'p-4 space-y-3']">
          <button v-if="hasActiveFilters()" @click="clearAllFilters" :class="buttonSecondaryClasses"
            class="w-full py-3 text-sm text-center transition-colors border rounded-lg">
            Clear All Filters
          </button>
          <button @click="applyMobileFilters" :class="buttonPrimaryClasses"
